...
Excited to announce that the throne WHOIS module is powered by the [throne API](/docs/api/api)! The throne API is leveraging RDAP resources from various registrars and IANA to provide the proper readable format to the user. RDAP in my opinion is a horrible "protocol", the whole vCardArray thing is absurd.
