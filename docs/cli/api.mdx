...
Commands:
  setapi  Use this command to login to the throne API and set your API key
```

### api setapi

Using `throne api setapi` you can retrieve your key from our authentication servers. This is in the form of a Bearer token, this command will automatically create our config file and set your Bearer token within the file.

You may run this command directly and be prompted for credentials OR you can use options. See both examples below.

```bash title="Prompt"
[user@throne ~]$ throne api setapi
...
:::caution API Scopes
Unless you've already been instructed by throne to use a scope, always choose N (or n) when being prompted for scope. If you choose Y (or y) and set a scope it will have no effect.
:::

```bash title="Options"
[user@throne ~]$ throne api setapi -u testuser@throne.dev -p Sup3rS3cr3tS3cur3P@55w0rd!
Successfully set throne API key.
```
...
throne_key: Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Inp1Z3RDRWtPM2hvNVIwM3YxbHJZeSJ9eyJpc3MiOiJodHRwczovL3Rocm9uZS1wcm9kLnVzLmF1dGgwLmNvbS8iLCJzdWIiOiJhdXRoMHw2MTI3YjE1YjhmYzRiYTAwNzFDExN2EiLCJhdWQiOiJodHRwczovL2FwaS50aHJvbmUuZGV2IiwiaWF0IjoxNjMwNDMxNTAxLCJl
shodan_key: MoRz6z4psAP0GMqiPXv3iSHqUUuzWFuq
```
