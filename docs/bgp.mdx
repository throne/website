...
```bash title="Help"
[user@throne ~]$ throne bgp look --help
Usage: throne bgp look [OPTIONS] ADDRESS_OR_PREFIX
...
We have one available option and an argument that is needed when using `throne bgp look`. 
...
can get out of the `throne bgp look` command.

```bash title="Example"
[user@throne ~]$ throne bgp look 1.1.1.1
Peer: 198.32.160.12
...
[user@throne ~]$ throne bgp look 1.1.1.1 --location DE
...
[user@throne ~]$ throne bgp look 8.8.8.0/24
Peer: 198.32.160.12
 Peer Location: New York City, New York, US
---
Origin AS: 15169
